            ...
            <h1 class="h3 mb-0 text-gray-800"><?php if(isset($dept)){showdept($dept,$conn);}  ?></h1>
                ...
                <form method="post" >
                        <!-- <div class="form-group">
                            ...
                            <select name="dept" id="dept" onchange="showUser(this.value)">
                              <?php 
                              //include_once("function/script.php");
                              //showprogramme($conn,$dept)
                             ?>
                            </select>
                        ...
                        <div class="form-group">
                            <label for="specialization">Programe:</label>
                            <select id="field" name="degree">
                         ...
                         <div class="form-group">
                            ...
                            <select id="field" name="special">
                              <option value="">Select Option</option>
                              <!-- This will be populated dynamically -->
                              <option value="927"> Test</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="specialization">Year of Entry:</label>
                        ...
                        <div class="form-group">
                            <label for="specialization">External:</label>
                        ...
                        <div class="form-group">
                            <label for="specialization">Year of Entry:</label>
                           <input type="date" id="effect" name="effect"/>
                        </div>
                       <!-- <div class="form-group">
...
if(isset($_POST['send']))
{
 ...
 $ext=$_POST['ext'];
 $effect=$_POST['effect'];
 header('location:../result.php?sec2='.$sec.'&degree='.$degree.'&special='.$special.'&ext='.$ext.'&effect='.$effect);
  //header('Location:../dashboard.php?p=result&sec2='.$sec.'&degree='.$degree.'&special='.$special.'ext='.$ext.'effect='.$effect);
}
...
$(document).ready(function() {
    $('#dept').change(function() {
        ...
        // AJAX request to fetch data based on the selected option
        $.ajax({
            url: 'getfield.php', // PHP file to handle the AJAX request
            type: 'POST',
            data: { q: selectedOption }, // Send the selected value to the server
            dataType: 'html', // Expecting HTML response
            success: function(response) {
                $('#field').html(response); // Populate the second dropdown with fetched data
            },
