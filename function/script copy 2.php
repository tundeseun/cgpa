<?php
    ...
    $FORCE_DEGREE_ID_UPDATE = false; // Set to false to disable the restriction
        ...
        if ($checkStmt) {
            ...
            if ($fetchSuccess && $count > 0 && ! empty($dept_new)) {
                ...
                if ($FORCE_DEGREE_ID_UPDATE) {
                    $missingDegreeQuery = "SELECT COUNT(*) FROM course_new WHERE dept_newids = ? AND (degree_id IS NULL OR degree_id = '' OR degree_id = 0) ";
        ...
        if (isset($excelFile) && $excelFile['error'] === UPLOAD_ERR_OK) {
            ...
            foreach ($worksheet->getRowIterator() as $row) {
                ...
                $checkQuerycoz = "SELECT id AS cgpa_id, status, unit, corder
                          FROM course_new
                          WHERE course_code = ? AND specialization = ? AND dept_newids = ? AND status2= ?";
                ...
                $checkStmtcoz->bind_param("ssss", $coz2, $field_of_interest, $dept, $status2);
                ...
                if (! $checkStmt1) {
                    ...
                    if (! $checkStmtcoz) {
                        ...
                        if (! $checkStmt3) {
                            ...
                            $checkQuery = "SELECT COUNT(*) FROM studentrecord WHERE matric = ? AND user_id = ? AND dept = ? AND specialization= ? ";
                            $checkStmt  = $conn->prepare($checkQuery);

                            if (! $checkStmt) {
                                echo "Prepare failed: (" . $conn->errno . ") " . $conn->error;
                            } else {
                                $checkStmt->bind_param("ssss", $rowData[0], $user_id, $department, $field_of_interest);
                                $checkStmt->execute();
                                $checkStmt->bind_result($count);
                                $checkStmt->fetch();
                                $checkStmt->close();
    ...
    function processresult($conn, $field, $effectivedate, $external, $dept, $resulttype, $mode)
    {
    ...
    FROM testscore t
    LEFT JOIN course_new c ON t.cozid = c.id LEFT JOIN programme_cgpa p ON t.degree = p.degree_id where t.field='$field' and t.effectivedate='$effectivedate' and t.external='$external' AND t.resulttype='$resulttype' AND t.mode = '$mode'
...
ORDER BY t.matric ASC";
        ...
        if (($result->num_rows > 0)) {
            ...
            foreach ($matrics as $matric) {
                ...
                $get_user_id    = "SELECT DISTINCT user_id  FROM `testscore` WHERE `matric` = '$matric'"; // Ordering by matric, course_code, and exam_sec
                $result_user_id = mysqli_query($conn, $get_user_id);
                $row_user_id    = mysqli_fetch_assoc($result_user_id);
                $user_id        = $row_user_id['user_id'];
...
<h4>$hod_name<br> $hod_desig</h4>
    ...
    function getStudentResults($conn, $matric, $effectivedate, $dept)
    {
        ...
        $query  = "SELECT course_new.course_code,studentrecord.name,testscore.id,testscore.cozid,testscore.cstatus,testscore.cunit,testscore.score FROM testscore INNER JOIN course_new ON course_new.id = testscore.cozid INNER JOIN studentrecord ON studentrecord.user_id = testscore.user_id WHERE testscore.matric = '$matric' AND testscore.effectivedate = '$effectivedate' AND testscore.dept = '$dept' AND course_new.status2 = 0 ORDER BY `course_new`.`course_code`";
        $result = mysqli_query($conn, $query);
    ...
    function processResultReal($conn, $dept, $degree, $field, $effectivedate, $external, $resulttype, $sec)
    {
        ...
        $sql = "SELECT t.matric,t.user_id,t.yr_of_entry,t.mode, t.score, t.exam_sec,t.field , t.external ,t.effectivedate, t.mode, c.course_code,c.corder, c.unit, c.status,p.type as program
        FROM testscore t
        LEFT JOIN course_new c ON t.cozid = c.id LEFT JOIN programme_cgpa p ON t.degree = p.degree_id where t.field='$field' and t.effectivedate='$effectivedate' and t.external='$external' ORDER BY c.corder"; // Ordering by matric, course_code, and exam_sec
        ...
        if (($result->num_rows > 0)) {
            ...
            foreach ($matrics as $matric) {
                ...
                $cgpa = ($unit[$matric] !== 0) ? number_format($tgpSum / $unit[$matric], 2) : '-';
                // var_dump($tupValues[$matric]);
                // var_dump($corepassed[$matric]);
                // var_dump($tctp);
                // var_dump($requiredpassed[$matric]);
                // var_dump($trtp);
                //&& ($tupValues[$matric] >= $ttp)
                if (($program == 'Academics') && ($tupValues[$matric] > 29) && ($corepassed[$matric] >= $tctp) && ($requiredpassed[$matric] >= $trtp)) {
                    if ($cgpa < 9.0 && $cgpa >= 5.0) {
            ...
            $checkQuery = "SELECT DISTINCT status,fac,field FROM testscore WHERE
    dept = '$dept' AND degree = '$degree' AND field = '$field' AND
    effectivedate = '$effectivedate' AND external = '$external'";
            $result = mysqli_query($conn, $checkQuery);if (! $result) {
            ...
            if ($status == 0) {
                $updatedSql   = "UPDATE testscore SET session_of_grad = '$sec', resulttype = '$resulttype' WHERE dept = '$dept' AND degree = '$degree' AND field = '$field' AND effectivedate = '$effectivedate' AND external = '$external'";
