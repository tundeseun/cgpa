...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['department'], $_GET['degree'], $_GET['field'], $_GET['effectivedate'], $_GET['board_approve'],$_GET['mode'],$_GET['external'],$_GET['resulttype'],$_GET['sec'],$_SESSION["user"])) {
  ...
  $boardDate = date('Y-m-d'); // Current timestamp for board_date


  // Use prepared statements to prevent SQL injection
  $sql = "UPDATE testscore SET `status` = 1, stage = 4, board_date = ? WHERE effectivedate = ? AND dept = ? AND degree = ? AND field = ? AND mode = ? AND `external` = ? AND resulttype = ? AND session_of_grad = ?";
  $stmt = $conn->prepare($sql);

  if ($stmt) {
    $stmt->bind_param('sssssssss', $boardDate, $effectivedate, $dept_id, $degree_id, $field_id, $mode, $external, $resulttype, $sec);

    if ($stmt->execute()) {

      // Get affected students to insert into notify table
      $select_sql = "SELECT DISTINCT testscore.matric,testscore.user_id, testscore.effectivedate, testscore.approval, testscore.session_of_grad, remark.remark FROM testscore 
                    INNER JOIN remark ON testscore.user_id=remark.user_id 
                    INNER JOIN new n ON testscore.user_id = n.id
                    INNER JOIN reginvoice ON reginvoice.appno = n.numeration 
                    WHERE testscore.effectivedate = ? AND testscore.dept = ? 
                    AND testscore.degree = ? AND testscore.field = ? 
                    AND testscore.mode = ? AND testscore.`status` = 1 AND testscore.stage = 4 
                    AND testscore.`external` = ? AND testscore.resulttype = ? AND testscore.session_of_grad = ? 
                    AND reginvoice.amount_paid = reginvoice.amount_charge
                    AND reginvoice.amount_paid > 0";
      $select_stmt = $conn->prepare($select_sql);
      
      if ($select_stmt) {
        $select_stmt->bind_param('ssssssss', $effectivedate, $dept_id, $degree_id, $field_id, $mode, $external, $resulttype, $sec);
        $select_stmt->execute();
        $result = $select_stmt->get_result();
        
        // Insert into notify table for each affected student
        while ($row = $result->fetch_assoc()) {
          $matric = $row['matric'];
          $user_id = $row['user_id'];
          $effectivedate2 = $row['effectivedate'];
          $approval = $row['approval'];
          $remark = $row['remark'];
          $sec2 = $row['session_of_grad'];
          $description = "Result,Effective date, Approval date";
          $student_record = $remark.','.$effectivedate2.','.$approval;
          $current_timestamp = date('Y-m-d H:i:s');
          
          $insert_sql = "INSERT INTO notify (matric, student_record, description, date, sec, user_id) 
                         VALUES (?, ?, ?, ?, ?, ?)";
          $insert_stmt = $conn->prepare($insert_sql);

          $insert_sql_online = "INSERT INTO notify (matric, student_record, description, date, sec, user_id) 
                         VALUES (?, ?, ?, ?, ?, ?)";
          $insert_stmt_online = $conn_online->prepare($insert_sql_online);
          
          if ($insert_stmt && $insert_stmt_online) {
            $insert_stmt->bind_param('ssssss', $matric, $student_record, $description, $current_timestamp, $sec2, $user_id);
            $insert_stmt->execute();
            $insert_stmt->close();
          }
        }
      ...
      echo "<script>
        Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: 'Result locked successful and NOR generated!',
          timer: 2000
        }).then(function() {
...
//Reject & Unlock functionality
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['department'], $_GET['degree'], $_GET['field'], $_GET['effectivedate'], $_GET['board_reject'],$_GET['mode'],$_GET['external'],$_GET['resulttype'],$_GET['sec'],$_SESSION["user"])) {
  // Validate and sanitize inputs
  error_log("GET parameters received: " . print_r($_GET, true));
  $dept_id = mysqli_real_escape_string($conn, $_GET['department']);
  $degree_id = mysqli_real_escape_string($conn, $_GET['degree']);
  ...
  // Use prepared statements to prevent SQL injection
  $sql = "UPDATE testscore SET `status` = 0, stage = 5, board_date = ? WHERE effectivedate = ? AND dept = ? AND degree = ? AND field = ? AND mode = ? AND `external` = ? AND resulttype = ? AND session_of_grad = ?";
  $stmt = $conn->prepare($sql);
  ...
  if ($stmt) {
    ...
    echo "<script>
        Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: 'Result rejected & unlocked successful!',
          timer: 2000
        }).then(function() {
          window.location.href = 'index.php';
        });
...
$(document).ready(function() {
    ...
    function createResultItem(result, metadata) {
        const lockLink = (result.stage == 4) 
            ? `<span class='disabled-link'><i class="fas fa-lock fa-sm"></i> Locked</span>`
            : `<a href='index.php?department=${encodeURIComponent(metadata.departmentId)}&degree=${encodeURIComponent(result.degree_id)}&field=${encodeURIComponent(metadata.specializationId)}&effectivedate=${encodeURIComponent(result.effectivedate)}&mode=${encodeURIComponent(result.smode)}&external=${encodeURIComponent(result.externalId)}&resulttype=${encodeURIComponent(result.resulttype)}&sec=${encodeURIComponent(result.sec)}&board_approve=board_approve' class='danger' onclick='return confirmAction(event)'><i class="fas fa-lock fa-sm"></i> Approve & Lock</a>`;
        
        const rejectLink = (result.status == 1) 
            ? ``
            : `<a href='index.php?department=${encodeURIComponent(metadata.departmentId)}&degree=${encodeURIComponent(result.degree_id)}&field=${encodeURIComponent(metadata.specializationId)}&effectivedate=${encodeURIComponent(result.effectivedate)}&mode=${encodeURIComponent(result.smode)}&external=${encodeURIComponent(result.externalId)}&resulttype=${encodeURIComponent(result.resulttype)}&sec=${encodeURIComponent(result.sec)}&board_reject=board_reject' class='success' onclick='return confirmReject(event)'><i class="fas fa-times fa-sm"></i> Reject Result</a>`;
...
function confirmReject(event) {
    ...
    Swal.fire({
        title: 'Are you sure?',
        html: "You want to Reject this result?<br><br>" +
              "<b>Note:</b><br>" +
              "• This action is irreversible!<br>" +
              "• The department will have access to modify the result.",
        showCancelButton: true,
        confirmButtonColor: '#0a2b4f',
        cancelButtonColor: '#d33',
