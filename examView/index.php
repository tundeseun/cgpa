...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['department'], $_GET['degree'], $_GET['field'], $_GET['effectivedate'], $_GET['fac_approve'],$_GET['mode'])) {
  ...
  $sql = "UPDATE testscore SET `status` = 1, stage = 4, board_date = '$boardDate' WHERE effectivedate = ? AND dept = ? AND degree = ? AND field = ? AND mode = ?";
  ...
  if ($stmt) {
    ...
    echo "<script>alert('Statement preparation failed!'); window.location.href = 'index.php';</script>";
  }
...
-->
  ...
  <script>
    function confirmAction(event) {
      if (!confirm('Are you sure you want to update the test score?')) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            data.forEach(function(result, index) {
              let viewLink = `<a target='_blank' href='../ProcessedBoardResult/${result.faculty}/${result.department}/${result.specialization}/${result.resultT}/${result.mode}/BroadSheet.pdf' class='success'><i class="fas fa-eye fa-sm text-white-50"></i> View</a>`;
              let lockLink = `<a href='index.php?department=${result.dept_id}&degree=${result.degree_id}&field=${result.field_id}&effectivedate=${result.effectivedate}&mode=${result.smode}&fac_approve=fac_approve' class='danger' onclick='confirmAction(event)'><i class="fas fa-lock fa-sm text-white-50"></i> Lock</a>`;

              if (result.stage == 4) {
                lockLink = `<a class='disabled-link'><i class="fas fa-lock fa-sm text-white-50"></i> Locked</a>`;
